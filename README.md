...
- [Creating a bootable BBC DFS disk](#creating-a-bootable-bbc-dfs-disk)
...
9. To create a directly bootable BBC Micro DFS image, use **Save boot disk…**.
10. Use **Save source** or **Save As…** to download edited source.
...
- bootable BBC DFS SSD generation;
- source metadata for default origins and BBC BASIC loader setup;
...
After a successful assembly, creates a **200 KB single-sided BBC DFS SSD image** which can be booted directly on a BBC Micro or compatible emulator.

The generated disk contains `!BOOT` and one tokenised BBC BASIC program. The compiled machine-code binary is appended directly after the end of the BASIC program. The BASIC loader executes any source-supplied `; @basic ...` setup statements, copies a small overlap-safe 6502 relocation routine to high memory below the MODE 7 screen, moves the compiled binary to its assembled origin, and jumps to the selected entry point.

The dialog lets you set the DFS disk title and BASIC program filename and select the machine-code entry point by symbol name or address. See [Creating a bootable BBC DFS disk](#creating-a-bootable-bbc-dfs-disk).
...
Specifies the assembly origin used when the active source contains no explicit `ORG` directive. This allows one source file to remain relocatable while the build address is selected in the UI.

A source can provide a suggested value with `; @default-origin ...`; the field remains editable. See [Source metadata](#source-metadata).
...
Source metadata uses ordinary assembly comments. Metadata therefore emits **no 6502 bytes** and does not affect the machine-code binary.

Two metadata forms are currently recognised.

## `@default-origin`

A relocatable source that deliberately contains no `ORG` can suggest its normal build address:

```asm
; @default-origin &1900
```

When that source file is selected, the assembler copies `&1900` into the toolbar **Origin** field. The user can freely change the field before assembly.
...
`ORG` is an assembly directive and establishes the address in the source itself. `@default-origin` is only a UI default.

The accepted address forms are BBC hexadecimal (`&1900`), dollar hexadecimal (`$1900`) and a bare three- or four-digit hexadecimal address.

## `@basic`

`@basic` supplies BBC BASIC setup statements to the optional boot-disk generator:

```asm
; @basic ENVELOPE1,1,0,0,0,1,1,1,100,-10,0,-10,100,0
; @basic *FX15,0
```

These lines have **no effect on normal assembly** and are not included in `.bin` output. They are used only when **Save boot disk…** is selected.

Each `@basic` comment becomes one line in the generated tokenised BBC BASIC loader. The statements are inserted in source order and execute after the generated loader selects MODE 7 but before it relocates and enters the machine-code program.

`@basic` lines in files reached through `INCLUDE` are also collected. Their order is the same as the effective assembly/include order.

The intended use is pre-run setup which is naturally performed by BBC BASIC, for example:

- `ENVELOPE` definitions;
- `SOUND` setup;
- `VDU` commands;
- `*FX` or other `*` commands;
- simple variable assignments;
- other direct BBC BASIC statement keywords supported by the loader tokenizer.

For example:

```asm
; @default-origin &0E00
; @basic ENVELOPE1,1,-2,-1,0,5,10,20,126,-4,0,-4,126,0
; @basic VDU23,1,0;0;0;0;

start:
    ; machine code follows
```

A leading `*` command is stored as ASCII in the BASIC program. For normal BASIC statements, the statement keyword is converted to the corresponding BBC BASIC token and the remainder is preserved as BASIC source text. Colon-separated direct statements are accepted and strings are protected from colon splitting.

The loader tokenizer is intended for **startup/setup statements**, not as a second general-purpose BBC BASIC compiler. In particular, source which depends on complex tokenisation of BASIC line-number references or arbitrary control-flow should remain in a normal BBC BASIC source/program rather than being embedded through `@basic`.

Malformed or unsupported `@basic` text does not invalidate the 6502 machine-code build. It is reported when a boot disk is created, because the metadata affects only that output format.

---

# Creating a bootable BBC DFS disk

**Save boot disk…** creates a standard 200 KB BBC DFS `.ssd` image from a successful machine-code build.

## Disk contents

The generated image contains two files:
...
The DFS boot option is set so Shift-Break executes `!BOOT`. `!BOOT` selects BASIC and chains the generated program. The generated program can also be loaded or chained manually from BASIC.

The program file contains:

```text
[tokenised BBC BASIC loader]
[BASIC end marker]
[compiled machine-code binary]
```

The machine-code bytes are therefore carried in the same DFS file as the loader rather than requiring a separate binary file on disk.

## Loader sequence

The generated BASIC loader performs the following operations in order:

1. Selects MODE 7, giving a predictable high-memory layout.
2. Executes all `; @basic ...` statements collected from the source project.
3. Sets `HIMEM` below the temporary relocation routine.
4. Copies the relocation routine to `&7AE0`, immediately below the normal MODE 7 screen at `&7C00`.
5. Uses `TOP` to identify the machine-code bytes appended after the BASIC program.
6. Supplies source address, destination/assembly origin, length and entry point to the mover.
7. Calls the mover.
8. The mover copies the binary to the selected assembly origin and jumps directly to the selected entry point. It intentionally does not return to BASIC.

The relocation routine is overlap-safe: it chooses forward or backward copying as required, so the destination is allowed to overlap the source image. This is important when the final program occupies RAM which was temporarily being used by BASIC.

## Entry point

The boot-disk dialog accepts either a numeric address or a source symbol, for example:

```text
start
```

Using a symbol is normally preferable for relocatable source because the entry address follows the selected assembly origin automatically.

## Memory restrictions

The generated mover occupies:

```text
&7AE0-&7BF4
```

while relocation is in progress. The final machine-code destination must not overlap this range.

The current boot-disk generator is designed for ordinary BBC main RAM and requires the compiled image to end at or below `&7FFF`. The initially loaded BASIC program plus appended binary must also fit below the mover when loaded at the standard DFS BASIC `PAGE=&1900` layout.

These are loader restrictions only. A particular machine-code program can have additional restrictions because of its own zero-page, workspace, display-memory, stack, OS or hardware use.

## BASIC setup example

A program which requires custom sound envelopes can keep the setup next to its assembly source:
...
The two comments produce no bytes in the assembled binary. If a boot disk is requested, the generated BASIC program contains those two `ENVELOPE` statements before the relocation logic.
...
The optional `@basic` facility is a loader-setup mechanism rather than a complete BBC BASIC source compiler. It tokenises direct setup statements and star commands needed by the generated boot loader; complex BASIC programs should be maintained as BASIC programs rather than encoded through metadata comments.

The bootable SSD generator targets a standard BBC Micro main-RAM/DFS environment and uses `&7AE0-&7BF4` temporarily for its relocation routine. See [Creating a bootable BBC DFS disk](#creating-a-bootable-bbc-dfs-disk) for the exact memory constraints.
...
Version 1.4 adds source-level `; @basic ...` metadata. These statements are ignored by 6502 assembly but are inserted into the generated tokenised BBC BASIC loader when **Save boot disk…** is used. This allows startup operations such as `ENVELOPE` definitions to remain with the assembly source without changing the machine-code binary.

Version 1.3 added `; @default-origin ...` metadata so relocatable source can advertise a suggested origin without an `ORG` directive.

Version 1.2 added the optional bootable DFS SSD generator with a tokenised BASIC loader, appended machine-code payload and overlap-safe high-memory relocation routine.

Version 1.1 added **Save As…**, `Ctrl/Cmd+Shift+S`, deliberately unminified human-readable JavaScript and the **(ChatGPT assisted)** application credit.
